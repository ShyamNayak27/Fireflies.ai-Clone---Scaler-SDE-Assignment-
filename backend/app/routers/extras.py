...
async def remove_tag(
    meeting_id: int, tag_id: int, session: AsyncSession = Depends(get_session)
) -> Response:
...
async def list_comments(
    ...
    return [
        ...
        for c in await comments_service.list_comments(session, meeting_id=meeting_id)
    ]
...
async def delete_soundbite(
    soundbite_id: int, session: AsyncSession = Depends(get_session)
) -> Response:
